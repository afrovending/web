...
from utils.email_templates import (
    render_booking_created, render_booking_status,
    render_payment_released, render_order_status, render_weekly_report
)
...
async def stripe_connect_return(account_id: str, request: Request):
    ...
    try:
        ...
        # Redirect to vendor dashboard - FRONTEND_URL must be set in .env for proper deployment
...
async def generate_weekly_report_html(report: WeeklyReportData) -> str:
    """Generate HTML email for weekly analytics report using Jinja2 template"""
    return render_weekly_report(report.model_dump())
