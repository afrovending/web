...
import { addItem, deleteItemPhoto, updateItem } from "@/lib/api/items";
...
export default function ItemForm({ onClose, item }: ItemFormProps) {
  ...
  // For images already stored in DB
  const [existingImages, setExistingImages] = useState<{
    urls: string[];
    publicIds: string[];
  }>({ urls: [], publicIds: [] });

  // For newly added images (files)
...
const handleImagesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const removeExistingImage = async (idx: number, productId: number) => {
    const publicId = existingImages.publicIds[idx];

    try {
      // API CALL
      await deleteItemPhoto(productId, publicId);
